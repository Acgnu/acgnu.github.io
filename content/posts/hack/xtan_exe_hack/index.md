...
背景
---
本文接另一篇文章 [某琴吧Flash播放器破解记录](../flash)
...
**在文章 [某琴吧Flash播放器破解记录](../flash) 中, 使用`getURL`生成的乐谱信息url去请求乐谱服务器得到的返回值里, 是包含`.ypa2`格式的资源地址的, 只不过现在`.ypa2`资源已经被删了, 不过观察刚才免费的乐谱`.ypdx`资源地址后, 会发现这和`.ypa2`的资源地址高度相似! 于是我将返回的`.ypa2`资源地址修改后缀名为`.ypdx`进行请求之后, 顺利得到了`.ypdx`格式的播放文件!**
